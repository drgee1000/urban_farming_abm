...
public class DefaultAgent implements IAgent {
		...
		if (this.health < this.healthThreshold) {
			...
			case 4: {
				...
				if (this.route == null) {
					...
					this.origin = ContextManager.getAgentGeometry(this).getCoordinate();
					this.destination = ContextManager.residentialProjection.getGeometry(R).getCentroid()
							.getCoordinate();
				}
	...
	public void calculateDis() {
		...
		// Iterable<Candidate1> iter_can1 =
		// ContextManager.candidate1Context.getObjects(Candidate1.class);
		...
		// Coordinate transfer =
		// ContextManager.candidate1Projection.getGeometry(can).getCentroid().getCoordinate();
	...
	public void calDisDes2Can() {
		...
		//
		// Candidate1 can = iter.next();
		// Coordinate can_coord =
		// ContextManager.candidate1Projection.getGeometry(can).getCentroid().getCoordinate();
		// distancesDes2Can[i] = e.orthodromicDistance(this.destination.x,
		// this.destination.y, can_coord.x, can_coord.y) / 1000;
